...
export function buildHabitResponse(habit, completions, options = {}) {
  ...
  return {
    ...
    recent_completions: normalizedCompletions
      ...
      .map((completion) => buildCompletionResponse(completion, habit)),
  };
}

export function buildCompletionResponse(completion, habit = null) {
  return {
    id: completion.id,
    ...
    is_complete: habit ? isCompletionComplete(completion, habit) : completion.count > 0,
  };
}
...
function lastCompletedAt(habit, completions) {
  const found = completions.find((completion) => isCompletionComplete(completion, habit));
  return found?.date ?? null;
}

function isCompletionComplete(completion, habit) {
  if (habit.target_type === "count") {
    return completion.count >= habit.target_count;
  }
